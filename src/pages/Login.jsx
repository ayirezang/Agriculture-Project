...
import { LockKeyhole, Sprout, UserRound, ArrowRight, ShieldCheck } from "lucide-react";
...
export default function Login({ onLogin }) {
  ...
  const submit = (e) => {
    ...
    const name =
      email.split("@")[0]
        .replace(/[._-]+/g, " ")
        .replace(/\b\w/g, (c) => c.toUpperCase()) || "Farmer";
    ...
    <main className="min-h-screen bg-[#f7faf8] px-4 py-8 sm:px-6">
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-6xl items-center gap-10 lg:grid-cols-2">
          ...
          <div className="mesh rounded-[2.5rem] border border-emerald-100 p-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-emerald-600">AI Marketplace</p>
              </div>
            </div>
            ...
            <p className="mt-5 max-w-xl text-base leading-7 text-slate-600">
              Your AI sales agent searches buyers, ranks offers and prepares bounded negotiations so you can focus on your farm.
            </p>
            <div className="mt-8 grid gap-3">
              {["Verified buyer network", "AI-assisted matching", "Farmer-controlled price limits"].map((item) => (
        ...
        <section className="mx-auto w-full max-w-md rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-900/5 sm:p-8">
          ...
          <p className="text-xs font-bold uppercase tracking-[.16em] text-emerald-600">Welcome back</p>
          <h2 className="mt-2 text-3xl font-black tracking-tight">Sign in to your farm workspace</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">Access your listings, buyer matches and AI sales agent.</p>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            ...
            <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700">
              Sign in <ArrowRight size={17} />
            </button>
          </form>

          <button onClick={demoLogin} className="mt-3 w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-100">
            Continue with demo farmer
          </button>

          <p className="mt-6 text-center text-[11px] leading-5 text-slate-400">
            MVP authentication is frontend-only. Connect Firebase, Supabase or your backend before production.
          </p>
        </section>
      </div>
    </main>
